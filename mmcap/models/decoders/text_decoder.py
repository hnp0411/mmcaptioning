...
class Transformer(nn.Module):
    ...
    def __init__(self, 
        ...
        self.embeddings = DecoderEmbeddings(vocab_size,
                                            d_model, # hidden_dim
                                            pad_token_id,
...
class TextDecoder(nn.Module):
    ...
    def forward(self, src, mask, pos_embed, tgt, tgt_mask):
        return self.transformer(src, mask, pos_embed, tgt, tgt_mask)

    def init_weights(self, pretrained:dict):
        if pretrained is not None:
            assert isinstance(pretrained, dict)
            logger = get_root_logger()
            print_log(f'load decoder weight from: {pretrained}',
                      logger=logger)
            self.transformer.init_weights(pretrained=pretrained['transformer_pretrained'])
